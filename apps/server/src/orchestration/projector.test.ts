...
describe("orchestration projector", () => {
  ...
  effectIt.effect("preserves the original running turn start across repeated session updates", () =>
    Effect.gen(function* () {
      const startedAt = "2026-02-23T08:00:05.000Z";
      const planUpdatedAt = "2026-02-23T08:00:20.000Z";
      let model = createEmptyReadModel("2026-02-23T08:00:00.000Z");
      const session = (status: "running", updatedAt: string) => ({
        threadId: "thread-timer",
        status,
        providerName: "droid",
        providerSessionId: "session-timer",
        providerThreadId: "provider-thread-timer",
        runtimeMode: "full-access",
      ...
      model = yield* projectEvent(
        ...
        makeEvent({
          ...
          type: "thread.created",
          aggregateKind: "thread",
          aggregateId: "thread-timer",
          occurredAt: "2026-02-23T08:00:00.000Z",
          commandId: "cmd-timer-create",
          payload: {
            threadId: "thread-timer",
            projectId: "project-1",
            title: "Timer",
            modelSelection: { provider: "droid", model: "default" },
            runtimeMode: "full-access",
            branch: null,
            worktreePath: null,
      ...
      model = yield* projectEvent(
        ...
        makeEvent({
          ...
          aggregateId: "thread-timer",
          occurredAt: startedAt,
          commandId: "cmd-timer-start",
          payload: { threadId: "thread-timer", session: session("running", startedAt) },
        }),
      );
      model = yield* projectEvent(
        model,
        makeEvent({
          ...
          aggregateKind: "thread",
          aggregateId: "thread-timer",
          occurredAt: planUpdatedAt,
          commandId: "cmd-timer-plan-update",
          payload: { threadId: "thread-timer", session: session("running", planUpdatedAt) },
        }),
      );
